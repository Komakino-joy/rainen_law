...
import {
  httpGetDistinctCityOptions,
  httpGetDistinctPropertyAssignOptions,
  httpGetDistinctPropertyStatusOptions,
  httpGetDistinctPropertyTypeOptions,
} from "@/services/http";
...
export const PropertiesContextProvider = ({ children }: { children: any }) => {
  ...
  const [propertiesSelectOptions, setPropertiesSelectOptions] =
    useState<propertiesSelectOptions>({
      p_city: [],
      p_type: [],
      p_status: [],
      p_assign: [],
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const httpFetchProperyInfo = async () => {
        ...
        const propertiesSelectOptions = Object.keys(
          propertiesResponseMap
        ).reduce((acc: any, fieldName: any) => {
          acc[fieldName] = propertiesResponseMap[fieldName as propKey].map(
            (field: any) => ({
