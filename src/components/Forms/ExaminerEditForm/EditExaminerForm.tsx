...
import {
  httpPostInsertExaminer,
  httpPostSelectedExaminer,
...
const EditExaminerForm: React.FC<EditEditExaminerFormProps> = ({
  ...
  const {
    ...
    defaultValues: async () => {
      if (selectedId) {
        ...
        const { id, name, code, type, compensate, is_active } = examinerInfo;
  ...
  const onSubmit = async (data: any) => {
    ...
    if (queryType === "update") {
      const updatedRecord = await httpPostUpdateExaminer({
        id: examinerId,
        data,
      });
      reset(updatedRecord);
